...
const SendOtpHandle = async (email: string) => {
  ...
  await AxiosHttp.getInstance().post('/auth/send-otp',
    ...
    if (error?.message == 'EMAIL_IS_USED') {
      throw new Error('Email đã tồn tại vui lòng đăng nhập hoặc email khác')
    }
  ...
  return (
    <>
      ...
      <div className='w-full'>
        ...
        <form className='mt-5 w-2xs flex flex-col gap-2' onSubmit={handleSubmit(onSubmit)}>
          ...
          {isError ? <Label variant='failed' size='sm' className='inline'>{error}</Label> : ""}

          <button disabled={isPending} className={`bg-black text-white font-bold p-3 hover:scale-[0.98] transition cursor-pointer flex items-center justify-center gap-2 w-full ${isPending && "bg-gray-500"}`}>
            Xác nhận
